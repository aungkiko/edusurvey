...
-- ระบบแบบสำรวจตามตัวชี้วัดแผนพัฒนาการศึกษาจังหวัดปัตตานี
-- พ.ศ. 2569-2573
-- ===================================================
...
CREATE TABLE `response_q9` (
    ...
    `has_mattayom_pak` TINYINT(1) NOT NULL DEFAULT 0,
    `mattayom_income` INT NOT NULL DEFAULT 0,
    `mattayom_total` INT NOT NULL DEFAULT 0,
    `has_vocational` TINYINT(1) NOT NULL DEFAULT 0,
    `vocational_income` INT NOT NULL DEFAULT 0,
    `vocational_total` INT NOT NULL DEFAULT 0,
    `has_associate` TINYINT(1) NOT NULL DEFAULT 0,
    `associate_income` INT NOT NULL DEFAULT 0,
    `associate_total` INT NOT NULL DEFAULT 0,
    `notes` TEXT NULL,
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY `uq_response` (`response_id`),
    FOREIGN KEY (`response_id`) REFERENCES `survey_responses`(`id`) ON DELETE CASCADE
